...
from ape_blockscout.exceptions import UnsupportedEcosystemError, get_request_error
from ape_blockscout.utils import API_KEY_ENV_VAR_NAME, BLOCKSCOUT_URI


def get_blockscout_uri(ecosystem_name: str, network_name: str):
    if ecosystem_name == "ethereum" and network_name == "mainnet" :
        return f"{BLOCKSCOUT_URI}eth/mainnet"
    elif ecosystem_name == "ethereum-classic" and network_name == "mainnet":
        return f"{BLOCKSCOUT_URI}etc/mainnet"
    elif ecosystem_name in ["poa","xdai"]:
        return (
            f"{BLOCKSCOUT_URI}{ecosystem_name}/{network_name}/"
        )
    raise UnsupportedEcosystemError(ecosystem_name)


def get_blockscout_api_uri(ecosystem_name: str, network_name: str):
    if ecosystem_name == "ethereum" and network_name == "mainnet":
        return f"{BLOCKSCOUT_URI}eth/mainnet/api" 
    elif ecosystem_name == "ethereum-classic" and network_name == "mainnet":
        return f"{BLOCKSCOUT_URI}eth/mainnet/api" 
    elif ecosystem_name in ["poa","xdai"]:
        return f"{BLOCKSCOUT_URI}{ecosystem_name}/{network_name}/api"
...
class _APIClient:
    ...
    def base_uri(self) -> str:
        return get_blockscout_api_uri(self._ecosystem_name, self._network_name)
